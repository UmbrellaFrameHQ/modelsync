...
`--version` kaldırılırsa NuGet’teki mevcut kararlı sürüm yüklenir. Bu belge 1.4.0-rc.1 API’sine göre hazırlanmıştır.
...
Varsayılan kolon adı property adıdır. ModelSync 1.4.0-rc.1 ile `DbColumnName` database kolon adını değiştirebilir, `DbIgnore` ise public yardımcı property'leri schema discovery dışında bırakabilir.
...
ModelSync 1.4.0-rc.1 kullanırken aşağıdaki kuralları zorunlu süreç kabul edin:
...
ModelSync 1.4.0-rc.1 `DbIgnore` ve `DbColumnName` desteği içerir. Database kolonu olmayacak hesaplanmış public property'ler `DbIgnore` ile schema discovery dışında bırakılabilir.
...
- ModelSync 1.4.0-rc.1 paketleri `DbIgnore` ve `DbColumnName` ile public property ignore ve column-name override desteği içerir.
...
Teknik olarak evet; ancak şema modellerini ayrı tutmak hâlâ daha güvenlidir. ModelSync 1.4.0-rc.1 `DbIgnore` desteğiyle yardımcı property'leri hariç tutabilir.
