...
Kurulum, model tanımlama, SQL üretimi, DDL yürütme, migration, stored procedure ve production kullanımı.
...
16. [Hata yönetimi ve troubleshooting](#hata-yönetimi-ve-troubleshooting)
17. [Test yaklaşımı](#test-yaklaşımı)
18. [Production kullanım rehberi](#production-kullanım-rehberi)
19. [Tam örnek proje yapısı](#tam-örnek-proje-yapısı)
20. [API hızlı referans](#api-hızlı-referans)
21. [Sürüm 1.0.7 sınırları](#sürüm-107-sınırları)
22. [Sık sorulan sorular](#sık-sorulan-sorular)
23. [Sonuç](#sonuç)

# Belge hakkında

Bu kılavuz, **ModelSync 1.0.7** paketlerini NuGet üzerinden yükleyen bir .NET geliştiricisinin projeyi kaynak koda bakmadan doğru biçimde kullanabilmesi için hazırlanmıştır. Kurulumdan başlayarak model tanımlama, SQL üretme, tablo oluşturma, indeks yürütme, kolon işlemleri, migration scriptleri, stored procedure senkronizasyonu, dependency injection, logging, analyzer, test ve production güvenliği ele alınır.
...
| Model değişince canlı veritabanını otomatik tam diff etme | 1.0.7’de yoktur. Açık kolon operasyonları veya SQL migration dosyaları kullanılmalıdır. |
...
`--version` kaldırılırsa NuGet’teki mevcut kararlı sürüm yüklenir. Bu belge 1.0.7 API’sine göre hazırlanmıştır.
...
### SQL Server 1.0.7 kullanım notu
...
Özellikle 1.0.7’de generic async üretim yerine bu çağrının ardından async yürütme kullanmak daha güvenlidir:
...
- 1.0.7’de `GenerateTruncateTableSql<T>()` temel sınıftan `TRUNCATE TABLE` üretir; SQLite bunu desteklemez. SQLite’da kendi `DELETE FROM "Table";` komutunuzu kullanın.
...
Kolon adı property adıdır. 1.0.7’de ayrı bir column-name override attribute’ü yoktur.
...
| SQLite | `SQLiteColumnPrimaryKey` | 1.0.7 `PRIMARY KEY AUTOINCREMENT` üretir; yalnız `INTEGER` kolonla kullanın. |
...
- Boşluk, tire, nokta veya schema-qualified ad kullanmayın; 1.0.7 foreign key snippet’i bu adları ayrıca quote etmez.
...
- Cascade seçenekleri için 1.0.7’de hazır attribute parametresi yoktur; migration scripti kullanın.
...
`ARRAY`, `RANGE` ve bazı extension tabanlı tiplerde üretilecek SQL’i mutlaka kontrol edin; 1.0.7 yalnız enum adını/uzunluğu birleştirir ve gelişmiş type declaration modellemesi yapmaz.
...
SQL Server 1.0.7’de provider-specific `ifNotExists` guard’ı için initializer’a doğrudan `SqlServerTableGenerator` enjekte edip `GenerateSqlServerTable<T>(true)` kullanın.
...
ModelSync 1.0.7 kullanırken aşağıdaki kuralları zorunlu süreç kabul edin:
...
8. 1.0.7’de batch/script/history işlemleri tüm provider’larda tek atomik transaction olarak garanti edilmez.
...
1.0.7 PostgreSQL overloaded procedure signature’larını desteklemez. Aynı schema ve adla farklı parametre listesine sahip procedure’ler kullanıyorsanız manuel migration yönetin.
...
ModelSync 1.0.7 tüm public property’leri kolon kabul eder. Ignore/NotMapped attribute’ü yoktur. Database kolonu olmayacak hesaplanmış public property’yi ayrı DTO’ya taşıyın veya public model yapısını ayırın.
...
### SQLite “near TRUNCATE” hatası

Neden: SQLite `TRUNCATE TABLE` desteklemez.
...
Şema modellerini domain entity veya API DTO’larından ayırmak, 1.0.7’de tüm public property’lerin kolon kabul edilmesi nedeniyle faydalıdır.
...
| `GenerateTruncateTableSql<T>()` | TRUNCATE SQL döndürür. |
...
# Sürüm 1.0.7 sınırları

## 77. Bilinmesi gereken güncel sınırlar

- Model-to-live-database tam şema diff yoktur.
...
Teknik olarak evet; ancak 1.0.7 tüm public property’leri kolon kabul ettiği için şema modellerini ayrı tutmak daha güvenlidir.
