...
`--version` kaldırılırsa NuGet’teki mevcut kararlı sürüm yüklenir. Bu belge 1.2.2 API’sine göre hazırlanmıştır.
...
ModelSync 1.2.2 kullanırken aşağıdaki kuralları zorunlu süreç kabul edin:
...
- ModelSync 1.2.2 paketleri `DbIgnore` ve `DbColumnName` ile public property ignore ve column-name override desteği içerir.
