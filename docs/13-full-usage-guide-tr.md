...
22. [CLI ve migration raporları](#cli-ve-migration-raporları)
23. [Sürüm 1.4.0 sınırları](#sürüm-140-rc1-sınırları)
24. [Sık sorulan sorular](#sık-sorulan-sorular)
25. [Sonuç](#sonuç)

# Belge hakkında

Bu kılavuz, **ModelSync 1.4.0** paketlerini NuGet üzerinden yükleyen bir .NET geliştiricisinin projeyi kaynak koda bakmadan doğru biçimde kullanabilmesi için hazırlanmıştır. Kurulumdan başlayarak model tanımlama, SQL üretme, tablo oluşturma, migration scriptleri, stored procedure senkronizasyonu, CLI, raporlama, test ve production güvenliği ele alınır.
...
| `UmbrellaFrame.ModelSync.Oracle` | Oracle preview tablo DDL ve kısmi güvenli model karşılaştırması | Preview olarak değerlendirin. |
| `UmbrellaFrame.ModelSync.Analyzers` | Model attribute hatalarını IDE ve build sırasında bulur | İsteğe bağlı, tavsiye edilir. |
| `UmbrellaFrame.ModelSync.Cli` | Terminal ve CI için validation, dry-run, execution ve raporlama | İsteğe bağlı .NET tool. |

Core ve kararlı provider paketleri `netstandard2.0`, Oracle preview `netstandard2.1`, CLI ise `net8.0` hedefler.
...
```bash
dotnet add package UmbrellaFrame.ModelSync.SqlServer --version 1.4.0
```

### MySQL / MariaDB

```bash
dotnet add package UmbrellaFrame.ModelSync.MySql --version 1.4.0
```

### PostgreSQL

```bash
dotnet add package UmbrellaFrame.ModelSync.PostgreSQL --version 1.4.0
```

### SQLite

```bash
dotnet add package UmbrellaFrame.ModelSync.SQLite --version 1.4.0
```
...
```bash
dotnet add package UmbrellaFrame.ModelSync.Analyzers --version 1.4.0
```

`--version` kaldırılırsa NuGet’teki mevcut kararlı sürüm yüklenir. Bu belge 1.4.0 API’sine göre hazırlanmıştır.
...
Varsayılan kolon adı property adıdır. ModelSync 1.4.0 ile `DbColumnName` database kolon adını değiştirebilir, `DbIgnore` ise public yardımcı property'leri schema discovery dışında bırakabilir.
...
| SQLite | `SQLiteColumnPrimaryKey` | `PRIMARY KEY AUTOINCREMENT` üretir; yalnız `INTEGER` kolonla kullanın. |
...
- Boşluk, tire, nokta veya schema-qualified ad kullanmayın; basit foreign key attribute yüzeyi bu adları ayrıca quote etmez.
...
- Cascade seçenekleri için hazır attribute parametresi yoktur; migration scripti kullanın.
...
`ARRAY`, `RANGE` ve bazı extension tabanlı tiplerde üretilecek SQL’i mutlaka kontrol edin; mevcut type attribute yüzeyi enum adını ve uzunluğu birleştirir, gelişmiş type declaration modellemesi yapmaz.
...
SQL Server `ifNotExists: true` seçeneğini generic ve provider-specific generator çağrılarında `OBJECT_ID` korumasıyla uygular. Provider’a özel API, okunabilirlik için yine tercih edilebilir.
...
| `SourceSql` | Kayıtlı kaynak script |
| `PlannedExecutionSql` | Otomatik çalıştırma için seçilen SQL |
| `RepairSql` | Yalnız inceleme amaçlı additive onarım önerileri |
| `UnappliedDrift` | Otomatik onarımın çözüldüğünü kanıtlayamadığı fark |
| `SqlToApply` | Planlanan execution SQL'i için uyumluluk alias'ı |
| `Reason` | Planın neden bu durumda olduğu |
| `HasChanges` | `ChangeType != None` |

Değiştirilmiş tablo scriptleri sessizce onarılıp tamamen uygulanmış sayılmaz. Best-effort parser eksik kolon bulursa plan `RepairSql` önerileri üretir, `RequiresManualReview` değerini açar, `PlannedExecutionSql` alanını boş bırakır ve tam script hash'ini ilerletmez. Öneriyi inceledikten sonra yeni bir migration oluşturun.
...
Reset tüm database'i etkileyebilecek yıkıcı bir işlemdir. Hedef database ve ortam kontrolü için structured seçenekleri kullanın:
...
var options = new MigrationRunnerOptions
{
    ...
    ResetOptions = new DatabaseResetOptions
    {
        ...
        BackupBeforeReset = true, // yalnız SQL Server
...
Açık izin verilmezse, database adı eşleşmezse, ortam allow-list içinde değilse veya hedef bir sistem database'i ise işlem başlamadan durur. SQL Server reset işlemini native lock alınmadan önce yapar; ardından readiness, infrastructure, history ve migration scriptleri normal lock altında devam eder.
...
Strict mode kullanırken stored procedure synchronizer dosyalarına `GO` eklemeyin. Açık `LegacyEmbeddedSql` compatibility profile, terminal `GO`, `SET ANSI_NULLS` ve `SET QUOTED_IDENTIFIER` içeren desteklenen deployment dosyalarını normalize edebilir; belirsiz executable side batch'ler yine bloklanır.

## 55. Migration production güvenlik kuralları

ModelSync 1.4.0 kullanırken aşağıdaki kuralları zorunlu süreç kabul edin:
...
8. Transaction atomikliği provider özelliğine ve seçilen `MigrationTransactionPolicy` değerine bağlıdır; destek matrisini kontrol edin.
...
PostgreSQL overloaded procedure signature’ları desteklenmez. Aynı schema ve adla farklı parametre listesine sahip procedure’ler kullanıyorsanız manuel migration yönetin.
...
- Strict mode stored procedure dosyasında SQL Server `GO` kabul etmez. Terminal `GO` ve session-setting batch'leri gereken desteklenen deployment dosyaları için açık `LegacyEmbeddedSql` profile kullanın.
- Dry-run için önce `Compare...` çağrısı yapın.

# Canlı model senkronizasyonu

Model synchronizer sınıfları, dry-run-first canlı veritabanı karşılaştırma katmanıdır.
...
ModelSync aynı çalıştırmada manuel ve otomatik tablo sahipliğini birlikte yönetebilir:
...
Tersi stratejide global davranışı automatic-safe tutup hassas tabloları manuel işaretleyebilirsiniz:
...
`ManualOnly` operasyonları `ManualOperations` altında raporlanır ve otomatik çalıştırılmaz. `ApplySafeChanges` yalnız güvenli, provider tarafından desteklenen ve dependency'leri hazır operasyonları uygular; destructive şema değişiklikleri bloklu kalır.
...
ModelSync 1.4.0 `DbIgnore` ve `DbColumnName` desteği içerir. Database kolonu olmayacak hesaplanmış public property'ler `DbIgnore` ile schema discovery dışında bırakılabilir.
...
## Bir milyon kayıt ölçek testi

İsteğe bağlı `UmbrellaFrame.ModelSync.ScaleTest` projesi aynı bir milyon kayıt senaryosunu SQL Server, MySQL, MariaDB, PostgreSQL, SQLite ve Oracle üzerinde çalıştırır. Katalog okuma, kararlı karşılaştırma, nullable kolon ekleme, indeks oluşturma, ikinci karşılaştırmada idempotency, destructive değişikliği engelleme ve desteklenen provider'larda migration history davranışı ölçülür.
...
Reset, native lock, stored procedure, trigger, seed, constraint ve foreign key gibi daha geniş özellikler normal provider integration projelerinde doğrulanır. Ölçek testinde görülen süreleri production garantisi olarak değerlendirmeyin; donanım, ağ, indeksler, database ayarları ve gerçek veri dağılımı DDL süresini değiştirir.
...
Şema modellerini domain entity veya API DTO’larından ayırmak hâlâ faydalıdır. Veritabanına yazılmaması gereken public property’leri `[DbIgnore]` ile açıkça hariç tutun; analyzer çakışan mapping attribute’larını derleme sırasında bildirir.
...
# CLI ve migration raporları

CLI'yi .NET tool olarak kurun:

```bash
dotnet tool install --global UmbrellaFrame.ModelSync.Cli --version 1.4.0
```

Bağlantı bilgisini process argümanına yazmadan önce dry-run alın, ardından açık `--apply` onayı verin:
...
`validate`, dosya keşfini ve duplicate ID'leri kontrol eder; SQL'in güvenli olduğunu kanıtlayan bir provider parser değildir. `--connection` geriye uyumluluk için kalır fakat process listesinde görünebilir. Ctrl+C migration cancellation akışına aktarılır.

# Sürüm 1.4.0 sınırları
...
- ModelSync 1.4.0 paketleri `DbIgnore` ve `DbColumnName` ile public property ignore ve column-name override desteği içerir.
- Oracle preview provider henüz migration runner, stored procedure, reset ve native lock özelliklerinin tamamını desteklemez.
- CLI validation, migration metadata ve duplicate ID kontrolüdür; provider SQL parser değildir.
...
Teknik olarak evet; ancak şema modellerini ayrı tutmak hâlâ daha güvenlidir. ModelSync 1.4.0 `DbIgnore` desteğiyle yardımcı property'leri hariç tutabilir.
